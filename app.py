...
# ── Supabase user store ─────────────────────────────────────────────────────
# Render = app runtime
# Supabase = persistent users / approvals
# GitHub = code only
#
# Set these in Render → Environment:
# SUPABASE_URL=https://svcgphohovsaweqqdgyo.supabase.co
# SUPABASE_KEY=sb_publishable_gcmoaJoTLH3d8keEe5TrWg_c9oCeASl

SUPABASE_URL = os.environ.get("SUPABASE_URL", "https://svcgphohovsaweqqdgyo.supabase.co").rstrip("/")
SUPABASE_KEY = os.environ.get("SUPABASE_KEY", "sb_publishable_gcmoaJoTLH3d8keEe5TrWg_c9oCeASl")
...
def _supabase_request(method: str, path: str, body=None):
    """Small Supabase REST client using only Python standard library."""
    ...
    try:
        ...
        raise RuntimeError(f"Supabase error {e.code}: {details}") from e


def load_users():
    """Load all users from Supabase as {username: user_dict}."""
    rows = _supabase_request(
        "GET",
        f'{SUPABASE_USERS_TABLE}?select=username,name,role,approved,suspended,password_hash,created_at&order=created_at.desc'
    ) or []
...
def save_users(users: dict):
    ...
    Upserts every user in the provided dict into Supabase.
    """
    rows = []
    ...
    if rows:
        _supabase_request(
            "POST",
            f"{SUPABASE_USERS_TABLE}?on_conflict=username",
            rows
        )
...
def get_user_record(username: str):
    ...
    rows = _supabase_request(
        "GET",
        f'{SUPABASE_USERS_TABLE}?username=eq.{username}&select=username,name,role,approved,suspended,password_hash,created_at&limit=1'
    ) or []
...
def create_user(username: str, name: str, password_hash: str):
    ...
    _supabase_request("POST", SUPABASE_USERS_TABLE, row)
...
def update_user(username: str, updates: dict):
    ...
    _supabase_request(
        "PATCH",
        f"{SUPABASE_USERS_TABLE}?username=eq.{username}",
        updates
    )
...
def register():
    ...
    if request.method == "POST":
        ...
        if len(name) < 2:
            ...
            try:
                existing_user = get_user_record(username)
                if existing_user:
...
def login():
    ...
    if request.method == "POST":
        ...
        if isinstance(user, dict) and check_password_hash(user.get("password_hash", ""), password):
            if user.get("suspended", False):
                return render_template("login.html", error="Your access is currently suspended. Contact the 2TR8DE team.")
...
def approve_user(secret, username):
    ...
    if username != "admin":
        update_user(username, {"approved": True, "suspended": False})
...
def account_status():
    ...
    return render_template("status.html", status_user=status_user, message=message, state=state)
